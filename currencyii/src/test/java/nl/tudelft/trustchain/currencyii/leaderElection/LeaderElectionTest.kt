...
class PayloadTest {
    ...
    fun electionPayloadSerializeTest() {
        ...
        assertEquals(
            payload.DAOid.decodeToString(),
            deserialized.first.DAOid.decodeToString()
        )
        assertEquals(
            payload.mostRecentSWBlock.type,
            deserialized.first.mostRecentSWBlock.type
        )
        assertEquals(
            payload.mostRecentSWBlock.rawTransaction.decodeToString(),
            deserialized.first.mostRecentSWBlock.rawTransaction.decodeToString()
        )
        assertEquals(
            payload.mostRecentSWBlock.publicKey.decodeToString(),
            deserialized.first.mostRecentSWBlock.publicKey.decodeToString()
        )
        assertEquals(
            payload.mostRecentSWBlock.sequenceNumber,
            deserialized.first.mostRecentSWBlock.sequenceNumber
        )
        assertEquals(
            payload.mostRecentSWBlock.linkPublicKey.decodeToString(),
            deserialized.first.mostRecentSWBlock.linkPublicKey.decodeToString()
        )
        assertEquals(
            payload.mostRecentSWBlock.linkSequenceNumber,
            deserialized.first.mostRecentSWBlock.linkSequenceNumber
        )
        assertEquals(
            payload.mostRecentSWBlock.previousHash.decodeToString(),
            deserialized.first.mostRecentSWBlock.previousHash.decodeToString()
        )
        assertEquals(
            payload.mostRecentSWBlock.signature.decodeToString(),
            deserialized.first.mostRecentSWBlock.signature.decodeToString()
        )
        assertEquals(
            payload.mostRecentSWBlock.timestamp.time,
            deserialized.first.mostRecentSWBlock.timestamp.time
        )
...
class LeaderElectionTest {
    ...
    fun init() {
        ...
        val context = mockk<Context>()
        community = CoinCommunity(context)
        val network = Network()
...
class LeaderSignProposalTests() {
    ...
    lateinit var retPK: ArrayList<String>

    val daoID: ByteArray = "x".repeat(64).toByteArray()

    val serializedPayload: ByteArray = "y".repeat(64).toByteArray()

    val electionPacket: ByteArray = "e".repeat(64).toByteArray()

    val signPayload: SignPayload = mockk<SignPayload>()
    ...
    fun setUp() {
        ...
        every { community.serializePacket(any(), any()) } returns serializedPayload

        every { community.leaderSignProposal(any(), any(), any(), any()) } answers { callOriginal() }

        every { community.toastLeaderSignProposal(any()) } just runs
    }
    ...
    fun leaderSingProposalTest() {
        ...
        verify(exactly = 1) { community.sendPayload(peer1, electionPacket) }
        verify(exactly = 1) { community.sendPayload(peer3, electionPacket) }
        verify(exactly = 1) { community.sendPayload(peer4, electionPacket) }
    ...
    fun leaderSingProposalTestNotAllInDao() {
        ...
        verify(exactly = 1) { community.sendPayload(peer1, electionPacket) }
        verify(exactly = 1) { community.sendPayload(peer4, electionPacket) }
        verify(exactly = 0) { community.sendPayload(peer3, electionPacket) }
