...
open class SunCryptoService(context: Context? = null) : CryptoService() {
...
//        setKeyStore(InMemoryKeyStoreService())
    }
    ...
    override fun generateKey(algorithm: KeyAlgorithm): KeyId {
        ...
        val generator = when (algorithm) {
            KeyAlgorithm.EC -> keyPairGeneratorSecp256k1()
        ...
        val key = Key(newKeyId(), if (algorithm == KeyAlgorithm.ECDSA_Secp256k1) KeyAlgorithm.EC else algorithm, CryptoProvider.SUN, keyPair)
        keyStore.store(key)
        return key.keyId
    }
...
//            KeyAlgorithm.EdDSA_Ed25519 -> Signature.getInstance("Ed25519")
    ...
    override fun signEthTransaction(keyId: KeyId, encodedTx: ByteArray): ECDSASignature {
        val key = keyStore.load(keyId.id, KeyType.PRIVATE)
        when (key.algorithm) {
            KeyAlgorithm.EC -> return ECKeyPair.create(key.keyPair).sign(Hash.sha3(encodedTx))
