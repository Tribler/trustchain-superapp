...
class DidWeb(
    ...
    companion object {
      fun getDomain(didUrl: DidUrl) =
        didUrl.identifier.substringBefore(":").let { URLDecoder.decode(it, StandardCharsets.UTF_8.name()) }
      fun getPath(didUrl: DidUrl) =
        didUrl.identifier.substringAfter(":", "").split(":").map { part -> URLDecoder.decode(part, StandardCharsets.UTF_8.name()) }.joinToString("/")
