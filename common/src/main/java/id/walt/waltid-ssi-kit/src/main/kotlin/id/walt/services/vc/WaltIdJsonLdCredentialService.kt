...
open class WaltIdJsonLdCredentialService: JsonLdCredentialService() {
    ...
    override fun sign(jsonCred: String, config: ProofConfig): String {
        ...
        val key = keyStore.load(config.issuerDid, KeyType.PRIVATE)
        Log.e("Sign TEST", "key: $key, ${key.keyPair?.private}, ${key.keyPair?.public}")

        val signer = when (key.algorithm) {
            KeyAlgorithm.EC -> LdSigner.EcdsaSecp256k1Signature2019(key) // instead of key.keyId
            KeyAlgorithm.ECDSA_Secp256k1 -> LdSigner.EcdsaSecp256k1Signature2019(key) // instead of key.keyId
...
//        val proof = signer.sign(jsonLdObject)
    ...
    override fun verifyVc(issuerDid: String, vc: String): Boolean {
        ...
        val verifier = when (publicKey.algorithm) {
            KeyAlgorithm.EC -> id.walt.crypto.LdVerifier.EcdsaSecp256k1Signature2019(publicKey.getPublicKey())
...
//            else -> throw Exception("Signature for key algorithm ${publicKey.algorithm} not supported")
    ...
    override fun addProof(credMap: Map<String, String>, ldProof: LdProof): String {
        ...
        return JSONObject(HashMap<Any?, Any?> (signedCredMap)).toString()
//        return JsonObject(signedCredMap).toString()
    ...
    override fun listVCs(): List<String> {
        return waltIdInterface.vcCreatedDir.list()?.filter {
            it.endsWith(".json")
        }?.toList() ?: listOf()
