          ...
          field = OIDCProviderMetadata(
            ...
            .also {
              ...
              it.setCustomParameter("credential_manifests", listOf(
                ...
                )).map { net.minidev.json.parser.JSONParser(JSONParser.DEFAULT_PERMISSIVE_MODE).parse(Klaxon().toJsonString(it)) }
              )
            }
        }
      }
  ...
  fun getCredential(
    ...
    val resp = HTTPRequest(
      ...
      if (mode == CompatibilityMode.EBSI_WCT) {
        ...
        query =
          "did=${URLEncoder.encode(did, StandardCharsets.UTF_8.name())}" +
          "&type=${URLEncoder.encode(schemaId, StandardCharsets.UTF_8.name())}" +
          "&format=${format ?: "ldp_vc"}" +
          "&proof=${URLEncoder.encode(klaxon.toJsonString(proof), StandardCharsets.UTF_8.name())}"
      }
    }.also {
