...
import nl.tudelft.trustchain.common.ebsi.AndroidKeyStoreService
...
object DidService {
    ...
    val DEFAULT_KEY_ALGORITHM = EdDSA_Ed25519 // EC //ECDSA_Secp256k1 //EdDSA_Ed25519
    ...
    fun setAndroidKeyStoreService(aksService: AndroidKeyStoreService) {
        aksService.hashCode()
    }
...
//            else -> throw Exception("DID method $method not supported")
    ...
    fun load(did: String): Did {
        return load(DidUrl.from(did))
    }

    fun load(didUrl: DidUrl): Did {
        /*
        // Debugging code
        val loadedDid = loadDid(didUrl.did)
//        Log.e("DIDURL TEST", "Loaded did of ${didUrl.did}: ${loadedDid ?: "not loaded (null)"}")
        if (loadedDid != null) {
            val decoded = Did.decode(loadedDid)
            Log.e("DIDURL TEST", "Decoded did: ${decoded ?: "not decoded (null)"}")
        }*/
        return Did.decode(loadDid(didUrl.did)!!)!!
    }
...
//            DidUrl.generateDidEbsiV1DidUrl().did
    ...
    private fun createDidWeb(keyAlias: String?, options: DidWebOptions?): String {
        ...
        val domain = when (options.domain.isNullOrEmpty()) {
            ...
            else -> URLEncoder.encode(options.domain, StandardCharsets.UTF_8.name())
        }

        val path = when (options.path.isNullOrEmpty()) {
            true -> ""
            else -> ":${
                options.path.split("/").map { part -> URLEncoder.encode(part, StandardCharsets.UTF_8.name()) }.joinToString(":")
            }"
        }
