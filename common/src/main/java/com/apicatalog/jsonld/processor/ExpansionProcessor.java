...
public final class ExpansionProcessor {
    ...
    public static final JsonArray expand(Document input, final JsonLdOptions options, boolean frameExpansion) throws JsonLdError {
        ...
        final JsonStructure jsonStructure = input.getJsonContent();
        if (jsonStructure == null) throw new JsonLdError(JsonLdErrorCode.LOADING_DOCUMENT_FAILED, "Document is not pased JSON.");
        ...
        if (options.getExpandContext() != null) {
            ...
            final JsonStructure contextValue = options.getExpandContext().getJsonContent();

            if (contextValue != null) {
                activeContext = updateContext(activeContext, contextValue, baseUrl);
            }
        }
