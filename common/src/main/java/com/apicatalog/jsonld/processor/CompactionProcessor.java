...
public final class CompactionProcessor {
    ...
    public static final JsonObject compact(final Document input, final Document context, final JsonLdOptions options) throws JsonLdError {
        ...
        JsonValue contextValue = JsonValue.EMPTY_JSON_OBJECT;
        JsonStructure structure = context.getJsonContent();
        if (structure != null) {
            contextValue = JsonUtils.flatten(structure, Keywords.CONTEXT);
        }
