...
public final class FromRdfProcessor {
    ...
    public static final JsonArray fromRdf(final Document document, final JsonLdOptions options) throws JsonLdError {
        RdfDataset context = document.getRdfContent();
        if (context == null) throw new JsonLdError(JsonLdErrorCode.LOADING_DOCUMENT_FAILED, "Expected RDF document but got [" + document.getContentType() + "]");

        RdfToJsonld rdfToJsonld = RdfToJsonld.with(context);

        return rdfToJsonld.ordered(options.isOrdered())
