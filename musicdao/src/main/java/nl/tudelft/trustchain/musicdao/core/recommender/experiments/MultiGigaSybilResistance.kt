...
fun main() {
    ...
    val percentageAttackersList = (0..9 step 1).map { it.toDouble() / 10 }
    for(percentageAttackers in percentageAttackersList) {
        ...
        val fileOut = File("$contextDir/dataset/multi_giga_sybil_experiment_variable_beta_final.txt")
        val seed = 5
        val rng = Random(seed)
        fileOut.createNewFile()
    trustNetwork = TrustNetwork(subNetworks, "d7083f5e1d50c264277d624340edaaf3dc16095b")

    var allNodes = trustNetwork.nodeToNodeNetwork.getAllNodes().toList()
    ...
    totalNodes = allNodes.size
        ...
        for (betaDecay in betaDecayValues) {
            ...
            fileOut.appendText("PERCENT ATTACKERS: $percentageAttackers BETA DECAY: $betaDecay REPUTATION GAIN: $reputationGainForSybils")
            fileOut.appendText("\n")
