...
class DiscoveryCommunityTest {
    ...
    private fun createCommunity(): DiscoveryCommunity {
        ...
        val network = Network()
        return DiscoveryCommunity(myPeer, endpoint, network)
    }
    ...
    fun createSimilarityRequest() {
        ...
        val endpoint = getEndpoint()
        val network = Network()
        val community = spyk(DiscoveryCommunity(myPeer, endpoint, network))
        network.registerServiceProvider(community.serviceId, community)
        val peer = Peer(ECCrypto.generateKey(), Address("5.2.3.4", 5234))
    ...
    fun onSimilarityResponse_maxPeers() {
        ...
        val endpoint = getEndpoint()
        val network = Network()
        network.addVerifiedPeer(Peer(ECCrypto.generateKey(), Address("12.2.3.4", 5234)))
        val community = DiscoveryCommunity(myPeer, endpoint, network, 1)

        assertEquals(0, community.getPeers().size)
        assertEquals(1, network.verifiedPeers.size)

        val peer1 = Peer(ECCrypto.generateKey(), Address("13.2.3.4", 5234))
        val network1 = Network()
        val community1 = DiscoveryCommunity(peer1, endpoint, network1, 2)
        ...
        val peer2 = Peer(ECCrypto.generateKey(), Address("14.2.3.4", 5234))
        val network2 = Network()
        val community2 = DiscoveryCommunity(peer2, endpoint, network2, 2)
