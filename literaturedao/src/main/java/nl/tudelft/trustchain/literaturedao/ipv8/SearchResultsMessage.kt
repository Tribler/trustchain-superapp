...
data class SearchResultsMessage(val results : SearchResultList) : nl.tudelft.ipv8.messaging.Serializable {
    ...
    companion object Deserializer : Deserializable<SearchResultsMessage> {
        override fun deserialize(buffer: ByteArray, offset: Int): Pair<SearchResultsMessage, Int> {
            var toReturn = Gson().fromJson(buffer.toString(Charsets.UTF_8), SearchResultList::class.java)
